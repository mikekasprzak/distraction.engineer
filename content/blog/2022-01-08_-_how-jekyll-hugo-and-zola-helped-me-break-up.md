...
Okay, maybe not _"best"_, but in my mind "C++ with a C style" (or imperative C++) was the least-worst of the languages. I was never good at articulating _why_ I disliked other languages, but something bothered me about them, especially once they shifted to package managers.
...
My other problem was I was styling my blogs with HTML. I don't hate HTML, but as a native document format it's clumsy. I'd been using GitHub for years, and through it I became a fan of markdown. I rarely need the flexibility that raw HTML provides me.
...
For a while I was content with Jekyll, but the workflow was never as clean as WordPress. I took a break from blogging, but came back with no idea how to update my blog. There was a minor but essential step I could no longer remember, and I never found it mentioned in documentation or my own notes. I couldn't test locally. The only way I could blog now would be to write, push the changes, and hope GitHub Pages wouldn't choke on them.

In hindsight, I was beginning to understand why I disliked modern programming languages.
...
Trust is something I'm going to have to work on, but to my knowledge _trust_ was never a core tenant of Ruby. I don't even mean package managers as an attack vector (welcome to 20XX), but rather how seriously has the code in my chain of trust been vetted?
...
In my experience, working with Jekyll/Ruby/GitHub pages was unreliable. GitHub Pages is fine, but I'd make a change to my files, push it, and it would silently error. That said I believe GitHub did eventually expose GitHub Pages errors in GitHub environments. I don't doubt some people have had a positive experience with this workflow, but I came to hate updating my Jekyll blog thanks to it.

The way forward was to commit to learning Jekyll, Ruby, and Gem, but I didn't see value in Ruby.
...
I wasn't blogging anymore. My struggles with Jekyll and Ruby took the joy out of it. I had the itch though, and I need to write again.
...
But once I finished setting up, I didn't use it. I couldn't remember how.
...
Like before, I wasn't committed to learning Hugo and Go. I did try Go, but I didn't care for it. To make a post, I would have to re-setup and re-teach myself Hugo, then fiddle with GIT submodules. I wasn't looking forward to that. I also didn't have an _easy_ web interface to take the edge off. On top of all this, I was having a crisis of text editor. I wanted _desperately_ to move away from UltraEdit, a proprietary text editor I've used since the 90's. It was a series of "this just isn't working for me" events. To be fair there was a brand new pandemic going on, and many of us lacked motivation.
...
I'm a regular Ubuntu on _Windows Subsystem for Linux_ user (WSL), and I only saw a Snap package. You can't (couldn't?) use Snaps on WSL. I realize later there were other options, but in the moment I'm stuck.

Shit.

I didn't have time to research another generator, so I grabbed my older Ubuntu laptop, installed the Snap, and powered on.
...
Having taken the time to ship a Zola project first (Interactive Snacks website), learned Zola in depth, before coming up with a plan really helped. I'm not going to say I execute it flawlessly, but I have so much more confidence now. I am confident Zola will be _enough_ to handle the needs of both websites, more websites (this blog and the Academy), and I'm confident I can teach it to others.

I finally had my win.
...
Snap's and Flatpak's exist because of Linux distro fragmentation. Your Linux might ship with a different version of a library than mine. Sometimes that different version will work, but it's safer to assume not. Snap's and Flatpak's ship programs with exact copies of the libraries they require. There's a bit more nuance then that, but that's them in a nutshell.
...
The Rust developers decided nah, lets make static linking the default. That basically eliminates the need for Snap's and Flatpak's. Programs can be download and run like this was Windows or somethin'.

Lo and behold, the _right_ way for me to install Zola was to visit the [releases page](https://github.com/getzola/zola/releases), download and unzip the binary, and move it to a `/bin/` folder in my path.

No joke, this was inconceivable.

I think I've illustrated why you need to understand the environment, the package manager, and sometimes the language to use applications from an ecosystem.

Zola as a tool is great. I don't have much more to add, except that understanding it and its ecosystem is essential for getting the most out of it. That goes for any of these static website generators. You shouldn't use one unless you're willing to go _all in_.


## Feeling Rusty

Alright, I have a static website generator I like, but I have another problem: I'm not even using Rust, but it broke me. 
...
Honestly I can't decide if committee or chaos is the better approach here, but after 20 years of inaction, I'm ready for something new.
...
That's fascinating to me. It's as if [Valgrind](https://valgrind.org/) was built into the compiler, but it's so much more elegant than that.
...
Other things the Rust project does is it has [working groups](https://www.rust-lang.org/governance) focused on different use cases, one of those being Game Development. I even recognize some of those names.
...
The only problem is that Rust is a package manager language. You can sort-of work around it, but no, it's not practical. If you Rust, you're using and trusting packages.

I'll never be 100% comfortable trusting 3rd party code. Sometimes I don't always trust my own code, but I take responsibility for it. To Rust's credit, coding in Rust is so much more pedantic than any language I've used. It forces you to write safer code, or highlight the unsafe. You should always review 3rd part code before you use it, but if you're feeling lazy, at least you know their code had to pass the rigorous scrutiny of `rustc`.
