...
I eventually realized a support database is just a static website, like this blog. Hey! Zola would be perfect for building a support database! Not only does it display content, but once the data is on GitHub, **ANYONE** can contribute! Now that's more like it.

Okay, lets create a support website. Where does it go? What's the URL?

Right now the FreshDesk support portal is on `support.ldjam.com`. It's a CNAME alias to the actual support website on FreshDesk. It's not actually something we own.
...
Okay, we could move the rules page to `support.ldjam.com`. Side benefit: users could contribute fixes to the rules page. Awesome!
...
So alright. If the `/compo` website is finally going away, we can start talking about overhauling _all_ URL's.


## Today's ludumdare.com

`ludumdare.com` is the homepage. We've never done much with our homepages, we ran `ludumdare.com/compo` and `ldjam.com` instead. We should though. I threw together the [homepage](https://ludumdare.com) you see today _basically_ to make sponsors happy. They didn't exactly require it, but I thought it was important to have one. We seem more professional this way.
...
And hooray, Ludum Dare support will suck less.
